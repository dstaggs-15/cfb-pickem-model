/* docs/app.js
 * Front-end only — NO Python changes.
 * - Shows EXACT games in your custom order
 * - De-duplicates duplicate matchups (keeps last)
 * - Baked-in team colors with readable text
 * - Loads docs/data/fpi.json and displays FPI rank/value, favors, and AGREE/DISAGREE vs model pick
 * - Cache-busts JSON fetches to avoid stale GitHub Pages caching
 */
...
(async function () {
  ...
  try {
    ...
    if (res.ok) {
      const fpi = await res.json();
      fpiMeta = { source: fpi.source || 'FPI', updated: fpi.updated || fpi.as_of || '' };
      if (fpi && fpi.teams) {
        for (const [name, obj] of Object.entries(fpi.teams)) {
          fpiMap.set(normalize(name), {
            name,
            rank: (obj && typeof obj.rank === 'number') ? obj.rank : null,
            fpi:  (obj && typeof obj.fpi  === 'number') ? obj.fpi  : null
          });
          // optional aliases array in file
          if (obj && Array.isArray(obj.aliases)) {
            obj.aliases.forEach(alias => fpiMap.set(normalize(alias), {
              name,
              rank: (obj && typeof obj.rank === 'number') ? obj.rank : null,
              fpi:  (obj && typeof obj.fpi  === 'number') ? obj.fpi  : null
            }));
    ...
    console.warn('No/invalid fpi.json; skipping overlay.');
  }
  ...
  const percent = (x) => `${(Math.max(0, Math.min(1, Number(x))) * 100).toFixed(1)}%`;

  function fpiEntry(team) {
    return fpiMap.get(normalize(team)) || null;
    // returns { name, fpi, rank } or null
  }
  function favoredByFPI(home, away) {
    const H = fpiEntry(home), A = fpiEntry(away);
    if (!H && !A) return null;
    if (H && A) {
  ...
  function renderFPI(card, home, away, modelPick) {
    // if no FPI data for either, skip quietly
    const H = fpiEntry(home), A = fpiEntry(away);
    if (!H && !A) return;

    const fav = favoredByFPI(home, away);
    const agree = fav && modelPick ? (normalize(fav) === normalize(modelPick)) : null;
    ...
    wrap.innerHTML = `
      ...
      <div class="fpi-fav">
        ${fav ? `FPI favors: <strong>${fav}</strong>` : 'FPI favors: <strong>—</strong>'}
        ${agree === true  ? `<span class="fpi-badge agree">AGREE</span>` : ''}
        ${agree === false ? `<span class="fpi-badge disagree">DISAGREE</span>` : ''}
        ${fpiMeta && fpiMeta.updated ? `<span class="fpi-stamp">(${fpiMeta.source || 'FPI'} · ${fpiMeta.updated})</span>` : ''}
      </div>
    `;
  ...
  function renderOne(g) {
    ...
    const pHome = Number(g.model_prob_home || g.model_prob_home_raw || 0.5);
    const pAway = 1 - pHome;
  ...
  // optional search (#team-filter)
  const search = document.getElementById('team-filter');
  if (search) {
    search.addEventListener('input', () => {
      const q = normalize(search.value || '');
      if (!q) return render(ordered);
      const filtered = ordered.filter(g =>
        normalize(g.home_team).includes(q) || normalize(g.away_team).includes(q)
      );
      render(filtered);
    });
  }
