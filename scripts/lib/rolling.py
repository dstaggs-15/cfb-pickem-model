...
def build_sidewise_rollups(schedule, wide_stats, last_n, predict_df=None):
    ...
    # Robust UTC conversion for both naive and tz-aware datetimes
    schedule = schedule.copy()
    schedule['date'] = pd.to_datetime(schedule['date'], errors="coerce", utc=True)
    ...
    if predict_df is not None and not predict_df.empty:
        # Give future dates to predicted games to avoid contaminating historical order
        predict_df = predict_df.copy()
        if "date" not in predict_df.columns:
            predict_df["date"] = pd.Timestamp.utcnow() + pd.to_timedelta(365, unit="D")
        else:
            predict_df["date"] = pd.to_datetime(predict_df["date"], errors="coerce", utc=True)
        ...
        home_df = pd.concat([home_df, home_predict], ignore_index=True)
        away_df = pd.concat([away_df, away_predict], ignore_index=True)
