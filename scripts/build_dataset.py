...
def main():
    ...
    if 'id' in schedule.columns: schedule.rename(columns={'id': 'game_id'}, inplace=True)
    if 'gameId' in team_stats.columns: team_stats.rename(columns={'gameId': 'game_id'}, inplace=True)

    schedule['game_id'] = schedule['game_id'].astype(str)
    if not team_stats.empty: team_stats['game_id'] = team_stats['game_id'].astype(str)
    ...
    if "market_home_prob" not in X.columns: X["market_home_prob"] = np.nan
    ...
    # --- 7. Final Validation Step ---
    print("  Validating final feature matrix...")
    if train_df[final_feature_list].var().sum() < 1e-6:
        print("\nCRITICAL ERROR: Final feature matrix has no variance. The model will be useless.")
        sys.exit(1)
