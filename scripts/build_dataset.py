...
def main():
    ...
    # Create the entire feature set using the shared function
    X, diff_cols = create_feature_set(schedule, team_stats_long, venues_df, teams_df, talent_df, lines_df)

    # Fit market mapping and create market probability feature
    ...
    all_features = diff_cols + count_features + ENG_FEATURES_BASE + LINE_FEATURES + ["elo_home_prob", "market_home_prob"]
    final_feature_list = [f for f in all_features if f in X.columns]
